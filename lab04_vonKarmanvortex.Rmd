---
title: "Dimensional Analysis and the von Karman Vortex Street"
author: "David Kahler"
output: pdf_document
header-includes: 
      \usepackage{amsmath}
...
## Objectives  
In this computational/field experiment, you will investigate the oscillating vortex shedding behind an obstruction, similar to that explained above.  You will gain experience with measuring river flow velocity and making flow observations as well as running a computational fluid dynamics (CFD) simulation of the von Karman vortex street, which provides a computational laboratory to make observations and record data.  
...
We seek $5-3=2$ unique dimensionless variables, or $\Pi$ terms.  From experience, we begin with one $\Pi$ as the Reynolds number, Re, then we form the second.  
...
## Field Observations  
You will use a local stream as an experimental flume and PVC pipes as model obstructions.  Use an acoustic Doppler velocimeter (ADV) (Vector, Nortek, Boston, Massachusetts, USA) to measure the river flow in the vicinity of the obstruction and the oscillation on the lee side of the obstruction.  Because it is a natural experiment, you will only obtain one data point per pipe diameter per streamflow velocity.  You can change the velocity by moving to different parts of the stream.  Again, the goal is to plot the function,  $\mathrm{St} = \phi(\mathrm{Re})$.  

For this lab activity, please bring:
 - Lab notebook (anything will do) and pen
 - Calculator (not necessary, but could be convenient)
 - Waterproof boots or waders (boots will be sufficient, but bring whatever you are comfortable with.  The water where we will work is only a few inches.  Sandals are not appropriate).
